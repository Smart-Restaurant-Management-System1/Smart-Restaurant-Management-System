...
/**
 * Validates restaurant table edit fields (Capacity and Location)
 */
export function validateTableEditForm(formData) {
  const errors = {};
...
/**
 * Sanitizes input into strict payload for PUT /api/tables/{id}
 * Updates only capacity and location
 */
export function sanitizeTableEditPayload(formData) {
  const payload = {
    capacity: parseInt(formData.capacity, 10),
    location: (formData.location || '').trim(),
  };
  ...
  if (formData.status) {
    const statusTrimmed = formData.status.trim().toLowerCase();
    if (statusTrimmed === 'available') {
      payload.status = 'Available';
    } else if (statusTrimmed === 'occupied') {
